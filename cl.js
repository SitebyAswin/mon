...
const SYSTEM_FILES = [
    ...
    'config.json',
    'links.json',
    'pending_deletes.json' // <--- ADD THIS
];

const LINKS_FILE = path.join(DATA_DIR, 'links.json');

// ⭐ NEW: File to store pending deletes
const PENDING_DELETES_FILE = path.join(DATA_DIR, 'pending_deletes.json');

// Initialize if not exists
if (!fs.existsSync(PENDING_DELETES_FILE)) fs.writeFileSync(PENDING_DELETES_FILE, JSON.stringify({}, null, 2));

// Add it to system files so it doesn't get shown in folders
if (!SYSTEM_FILES.includes('pending_deletes.json')) SYSTEM_FILES.push('pending_deletes.json');
...
// Load once into memory on startup
if (fs.existsSync(USERS_FILE)) {
    try {
        ...
        if (Array.isArray(parsed.ids)) {
            usersData = parsed;
            usersData.count = parsed.ids.length; // Ensure count matches
        }
    } catch (err) {
...
const searchSession = {}; // <--- ⭐ NEW: Stores search results per user
let pendingPlaylist = {}; // Stores admin state for creating lists
...
bot.use(async (ctx, next) => {
    if (ctx.message) {
        ...
        scheduleDelete(ctx.chat.id, ctx.message.message_id, metaData.timers.user_msg);
    }
    await next();
});
...
function scheduleDelete(chatId, messageId, delayMs) {
    ...
    // 1. Save to Disk
    try {
        const db = JSON.parse(fs.readFileSync(PENDING_DELETES_FILE, 'utf8'));
        db[key] = expiryTime;
        fs.writeFileSync(PENDING_DELETES_FILE, JSON.stringify(db));
    } catch (e) { console.error("Save Timer Error:", e); }

    // 2. Set Memory Timer
    setDeleteTimer(chatId, messageId, delayMs);
}

// Internal function to actually set the timeout
function setDeleteTimer(chatId, messageId, delayMs) {
    setTimeout(async () => {
        try {
...
function restoreTimers() {
    if (!fs.existsSync(PENDING_DELETES_FILE)) return;
    ...
    try {
        ...
        console.log(`🔄 Restoring ${Object.keys(db).length} pending deletes...`);

        for (const [key, expiry] of Object.entries(db)) {
            const [chatId, msgId] = key.split(':');
            const remaining = expiry - now;

            if (remaining <= 0) {
                // Expired while bot was off -> Delete Immediately
                bot.telegram.deleteMessage(chatId, msgId).catch(()=>{});
                delete db[key];
                changed = true;
            } else {
                // Time still remaining -> Resume Timer
                setDeleteTimer(chatId, msgId, remaining);
            }
        }

        if (changed) fs.writeFileSync(PENDING_DELETES_FILE, JSON.stringify(db));
    } catch (e) { console.error("Restore Timers Error:", e); }
}
...
function trackUser(ctx) {
    ...
    if (!usersData.ids.includes(uid)) {
        usersData.ids.push(uid);
        usersData.count = usersData.ids.length;
        
        // Write to Disk (Async/Safe)
        try {
            fs.writeFileSync(USERS_FILE, JSON.stringify(usersData, null, 2));
...
function getStats() {
    // Return from memory directly
    return { users: usersData.count || 0, files: metaData.total_files || 0 };
}
...
// 🎨 Custom Icons & Breadcrumbs
function formatDisplayPath(relPath) {
    if (!relPath || relPath === '.' || relPath.trim() === '') return '<b>Main Collection</b>';

    const parts = relPath.split(/[/\\]/); // Split into folder names
    let currentPath = '';
    let htmlParts = [];
    
    // Ensure we have the bot username for the link
    const botUser = bot.botInfo ? bot.botInfo.username : ''; 

    // Add "Home" link
    htmlParts.push(`<a href="https://t.me/${botUser}?start=start">🏠 Home</a>`);
    ...
    parts.forEach((part, index) => {
        ...
        if (isLast) {
            // The current folder (Bold text, no link)
            htmlParts.push(`<b>${part}</b>`);
        } else {
            // Parent folder (Clickable Link)
            const id = getBreadcrumbLink(currentPath);
            const url = `https://t.me/${botUser}?start=${id}`;
            htmlParts.push(`<a href="${url}">${part}</a>`);
        }
    });

    // Join with a nice arrow
...
function rebuildCache(dir = DATA_DIR, relPath = '') {
    ...
    // 2. Process Files (⭐ NEW LOGIC)
    const files = items
        .filter(i => i.isFile() && i.name.endsWith('.json') && !SYSTEM_FILES.includes(i.name))
        .sort(sorter)
        .map(i => {
            const fName = i.name.replace('.json', '');
            let fIcon = FILE_ICON; // Default 📄
            
            // Read file briefly to check for 'icon' or 'type'
            try {
                const fData = JSON.parse(fs.readFileSync(path.join(dir, i.name), 'utf8'));
                
                if (fData.icon) fIcon = fData.icon; // Use custom icon if set
                else if (fData.type === 'redirect') fIcon = '↪️'; // Auto icon for redirects
                
            } catch(e) {}

            // Return Object instead of String
            return { name: fName, icon: fIcon };
        });
...
// 🔗 Helper: Auto-generate a deep link for a specific path
function generateDeepLink(ctx, relPath, type = 'file') {
    let links = {};
    ...
    let shortId = Object.keys(links).find(key => {
        const entry = links[key];
        return (entry.path === cleanPath || entry === cleanPath);
    });

    // 2. Create new if missing
    if (!shortId) {
        shortId = generateId();
        links[shortId] = { path: cleanPath, type: type, created_at: Date.now() };
        fs.writeFileSync(LINKS_FILE, JSON.stringify(links, null, 2));
    }

    return `https://t.me/${ctx.botInfo.username}?start=${shortId}`;
}

// 🍞 Helper: Get or Create a Deep Link for Breadcrumbs
function getBreadcrumbLink(targetPath) {
    const p = targetPath.replace(/\\/g, '/'); // Normalize path
    
    // 1. Load Links
    let links = {};
    try { links = JSON.parse(fs.readFileSync(LINKS_FILE, 'utf8')); } catch (e) {}

    // 2. Search for existing ID for this path (Reverse Lookup)
    const existingId = Object.keys(links).find(key => {
        const entry = links[key];
        return (entry.path === p || entry === p);
    });

    if (existingId) return existingId;

    // 3. Create New ID if missing
    const newId = generateId();
    links[newId] = { 
        path: p, 
        type: 'folder', 
        created_at: Date.now(),
        is_breadcrumb: true // internal flag
    };
...
// 🧹 Improved Filename Cleaner
function cleanFilename(rawName) {
    if (!rawName) return null; // Return null so we can handle fallback

    let name = rawName;
    // Remove File Extension
    name = name.replace(/\.[a-zA-Z0-9]{3,4}$/, "");
    // Remove Website domains and user tags
    name = name.replace(/(www\.[^\s]+|@[^\s]+|t\.me\/[^\s]+)/gi, "");
    // Remove Brackets [] and Parentheses ()
    name = name.replace(/\[.*?\]/g, "").replace(/\(.*?\)/g, "");
    // Remove Quality Tags
    const tags = ["1080p", "720p", "480p", "4k", "HEVC", "x264", "x265", "10bit", "HDR", "BluRay", "WEB-DL", "Dual Audio", "Hindi", "Eng", "Sub"];
    const tagRegex = new RegExp(`\\b(${tags.join('|')})\\b`, 'gi');
    name = name.replace(tagRegex, "");
    // Cleanup spaces
    name = name.replace(/[._]/g, " ").replace(/\s+/g, " ").trim();

    return sanitize(name) || null; // Ensure we don't return empty string
}
...
// 🔍 Recursive Search Helper with Ranking (FIXED: Ignores Quality Tags in Query)
function findPathRecursively(dummyDir, query, dummyRel = '') {
    let results = [];
    
    // 1. Clean the Query (Remove 1080p, 720p, etc.)
    let cleanQuery = String(query).toLowerCase();
    
    // List of tags to strip from the user's search input
    const searchTags = [
        "1080p", "720p", "480p", "2160p", "4k", "5k", "8k", "UHD", 
        "HEVC", "x264", "x265", "10bit", "HDR", "BluRay", "WEB-DL", "DVDRip", 
        "H264", "AAC", "AC3", "DTS", "5.1", "Dual Audio", "Hindi", "Eng", "Sub", "Esub"
    ];
    
    // Remove tags (e.g. "Iron Man 1080p" -> "Iron Man  ")
    const tagRegex = new RegExp(`\\b(${searchTags.join('|').toLowerCase()})\\b`, 'gi');
    cleanQuery = cleanQuery.replace(tagRegex, "");
    
    // Remove special chars and trim
    cleanQuery = cleanQuery.replace(/[._\-]/g, " ").trim();

    // 🛡️ If query becomes empty (e.g. user searched only "1080p"), return no results
    ...
    for (const [folderPath, content] of Object.entries(DIR_CACHE)) {
        ...
        const checkItem = (name, isDirectory) => {
            // 🔒 Check Ignore List (Season 1, 1080p folders, etc.)
            ...
            // 3. WORD BOUNDARY Match (e.g. "Lost" inside "The Lost World")
            else if (new RegExp(`\\b${escapeRegExp(cleanQuery)}\\b`).test(normName)) {
                score = 60;
            }
            // 4. GENERAL CONTAINS (Fuzzy)
            else {
                // Check if ALL remaining words exist in the filename
        ...
        // --- ⬇️ MODIFICATION START: Disable File Search ⬇️ ---
        
        // Check Files - (COMMENTED OUT TO SEARCH FOLDERS ONLY)
        /* 
        if (content.files) {
...
// 🔍 HELPER: Display Search Results with Pagination
function showSearchResults(ctx) {
    const session = searchSession[ctx.chat.id];
    const { results, page, query } = session;
    ...
    // Navigation Row
    const navRow = [];
    if (page > 0) navRow.push('◀️ Previous');
    if (page < totalPages - 1) navRow.push('▶️ More Results');
    ...
    return cleanReply(
        ctx, 
        `🔍 <b>Search Results:</b> "${query}"\n` +
        `📊 Found: <code>${totalCount}</code> items\n` +
...
// 🌐 Recursive HTML Generator (Fixed: Preview Badge is now a Link)
function generateHtmlTree(dir) {
    // 1. Load Links for Badges
    let linkMap = {}; 
    if (fs.existsSync(LINKS_FILE)) {
        try {
            const links = JSON.parse(fs.readFileSync(LINKS_FILE, 'utf8'));
            Object.keys(links).forEach(id => {
                const entry = links[id];
                const cleanP = (entry.path || entry).replace(/\\/g, '/');
                linkMap[cleanP] = id;
            });
        } catch(e) {}
    }
    ...
    items.forEach((item) => {
        ...
        const lookupPath = item.isDirectory() ? relPath : relPath.replace(/\.json$/, '');

        // Check for Deep Link Badge
        let deepLinkHtml = '';
        if (linkMap[lookupPath]) {
            const url = `https://t.me/${bot.botInfo.username}?start=${linkMap[lookupPath]}`;
            const linkClass = item.isDirectory() ? 'deeplink' : 'filelink'; 
            deepLinkHtml = ` <a href="${url}" target="_blank" class="tag ${linkClass}">[🔗 Link]</a>`;
        }

        if (item.isDirectory()) {
            // 📂 FOLDER
            let folderIcon = '📂'; 
            const configPath = path.join(dir, item.name, 'config.json');
            if (fs.existsSync(configPath)) {
                try {
                    const conf = JSON.parse(fs.readFileSync(configPath, 'utf8'));
                    if (conf.icon) folderIcon = conf.icon;
                } catch (e) {}
            }

            // ⭐ FIX: Read preview.json to make the link
            let previewHtml = '';
            const previewPath = path.join(dir, item.name, 'preview.json');
            ...
            if (fs.existsSync(previewPath)) {
                try {
                    ...
                    const cid = pData.from_chat_id || pData.chat_id;
                    const link = getSourceLink(cid, pData.message_id);
                    // Make it a clickable anchor tag
                    previewHtml = ` <a href="${link}" target="_blank" class="tag preview">[👁️ Preview]</a>`;
                } catch (e) {
                    // Fallback just in case
                    previewHtml = ` <span class="tag preview">[👁️ Preview]</span>`;
                }
            }

            html += `<li><span class="caret folder">${folderIcon} ${item.name}</span>${deepLinkHtml}${previewHtml}`;
            html += `<ul class="nested">` + generateHtmlTree(path.join(dir, item.name)) + `</ul>`;
            html += `</li>`;
            ...
            let icon = '📄';
            let cssClass = 'file';
            let displayName = item.name.replace('.json', '');

            try {
                const fPath = path.join(dir, item.name);
                const fData = JSON.parse(fs.readFileSync(fPath, 'utf8'));
                if (fData.icon) icon = fData.icon;

                if (fData.type === 'index') {
                    cssClass = 'playlist';
                    if (!fData.icon) icon = '📜'; 
                    extraHtml = ` <span class="tag playlist-tag">[List]</span>`;
                    
                    const cid = fData.from_chat_id || fData.chat_id;
                    const link = getSourceLink(cid, fData.message_id);
                    if (link) extraHtml += ` <a href="${link}" target="_blank" class="tag source">[📥 Source]</a>`;
                }
                else if (fData.type === 'redirect') {
                    icon = '↪️';
                    cssClass = 'redirect';
            ...
            html += `<li><span class="${cssClass}">${icon} ${displayName}</span>${deepLinkHtml}${extraHtml}</li>`;
        }
    });
...
function buildKeyboard(relPath, page = 0) {
    ...
    // UPDATED: Handle objects for both folders and files
    const folderBtns = folders.map(f => `${f.icon} ${f.name}`);
    
    // ⭐ Changed here: use f.icon instead of FILE_ICON
...
let candidates = [];
    const collectCandidates = (currentRel) => {
        ...
        // A. Add Files (⭐ UPDATED to handle Object)
        if (content.files) {
            content.files.forEach(f => {
                // f is now { name: "Iron Man", icon: "🎬" }
                candidates.push({ name: f.name, path: path.join(currentRel, f.name), type: 'file' });
            });
        }

        // B. Traverse Folders (Existing logic)
        if (content.folders) {
...
async function cleanReply(ctx, text, extra) {
    ...
    // USE DYNAMIC TIMER HERE 👇
    scheduleDelete(ctx.chat.id, sent.message_id, metaData.timers.bot_msg);
...
const sendHelp = (ctx) => {
    if (isAdmin(ctx)) {
        ...
        return ctx.replyWithHTML(
            ...
            `/mkdir &lt;Name&gt; - Create Folder (Use / for subfolders, | for multiple)\n` + // <--- UPDATED LINE
            `/mklist &lt;Name&gt; - Create list \n` +
...
bot.hears('▶️ More Results', (ctx) => {
    const session = searchSession[ctx.chat.id];
    if (!session || !session.results) return ctx.reply("❌ Search session expired. Please search again.");

    const totalPages = Math.ceil(session.results.length / ITEMS_PER_PAGE);
    if (session.page < totalPages - 1) {
        session.page++;
        return showSearchResults(ctx);
    } else {
        return ctx.reply("⚠️ End of results.");
    }
});

bot.hears('◀️ Previous', (ctx) => {
    const session = searchSession[ctx.chat.id];
    if (!session || !session.results) return ctx.reply("❌ Search session expired. Please search again.");
    ...
    if (session.page > 0) {
        ...
        return ctx.reply("⚠️ You are on the first page.");
    }
});
...
const absPath = getAbsPath(suggestion.path);
    ...
    // 1. 📂 If it is a FOLDER
    // We check type OR if the path exists and is a directory
    if (suggestion.type === 'folder' || (fs.existsSync(absPath) && fs.lstatSync(absPath).isDirectory())) {
         // ⭐ FIX: Use the helper to handle Previews, State & Icons automatically
         return openFolder(ctx, suggestion.path);
    }
    ...
    if (fs.existsSync(filePath)) {
        try {
            ...
            if (data.type === 'url') {
                ...
                scheduleDelete(ctx.chat.id, sentMsg.message_id, metaData.timers.file);
                return;
            }
            ...
            scheduleDelete(ctx.chat.id, sentMsg.message_id, metaData.timers.file);
            scheduleDelete(ctx.chat.id, timerMsg.message_id, metaData.timers.file);
...
// 🌐 NEW COMMAND: /fullhtree (Updated CSS)
...
<head>
    ...
    <style>
        ...
        li { margin: 6px 0; position: relative; line-height: 1.6; }
        
        /* Interactive Caret */
        .caret { cursor: pointer; user-select: none; font-weight: bold; color: #d35400; }
        .caret::before { content: "\\25B6"; color: black; display: inline-block; margin-right: 6px; font-size: 0.8em; }
        .caret-down::before { transform: rotate(90deg); }
        .nested { display: none; margin-left: 15px; border-left: 1px solid #ddd; padding-left: 10px; }
        .active { display: block; }

        /* Item Types */
        .file { color: #2980b9; margin-left: 22px; } 
        .playlist { color: #8e44ad; font-weight: 500; margin-left: 22px; } /* Purple for Playlists */
        .redirect { color: #d35400; font-style: italic; margin-left: 22px; }

        /* Badges */
        .tag { font-size: 0.75em; margin-left: 8px; padding: 2px 6px; border-radius: 4px; vertical-align: middle; text-decoration: none; display: inline-block;}
        
        .deeplink { background: #e8daef; color: #8e44ad; border: 1px solid #8e44ad; } /* Folder Links */
        .filelink { background: #d6eaf8; color: #2e86c1; border: 1px solid #2e86c1; } /* File/Playlist Links */
        
        .preview { background: #e8f6f3; color: #16a085; }
        .source { background: #f2f3f4; color: #7f8c8d; border: 1px solid #ccc; }
        .playlist-tag { background: #f4ecf7; color: #8e44ad; border: 1px solid #d2b4de; }
...
<body>
    ...
    <p><i>Click arrows to expand.</i></p>
    <ul id="myUL">${treeBody}</ul>
    <script>
        ...
        await ctx.replyWithDocument(
            ...
            { caption: `🌐 HTML Map for "${displayName}"` }
        );
        fs.unlinkSync(reportPath);
...
// ---------- Admin CRUD ----------
// 📂 MKDIR (Updated: Subfolders + Auto Emoji Detection)
bot.command('mkdir', (ctx) => {
    ...
    if (!rawText) return ctx.reply("❌ Usage: /mkdir Name1 | Name2/Subfolder");

    // Regex to find emojis
    const emojiRegex = /(\p{Emoji_Presentation}|\p{Extended_Pictographic})/u;

    // Split by '|' for multiple independent creations
    const folderChains = rawText.split('|').map(s => s.trim()).filter(s => s.length > 0);

    let created = [];
    let existing = [];

    folderChains.forEach(chain => {
        // Handle "Parent/Child" structure logic
        const parts = chain.split(/[/\\]/); 
        let tempPath = currentAbs; // Start tracking from current location

        parts.forEach(rawPart => {
            let icon = null;
            let namePart = rawPart.trim();

            // 1. Detect Emoji
            const match = namePart.match(emojiRegex);
            if (match) {
                icon = match[0]; // Capture the emoji (e.g. 🗓️)
                
                // 2. Remove the emoji from the name so it looks clean
                // e.g. "🗓️ 2025" -> "2025"
                namePart = namePart.replace(icon, '').trim();
            }

            // Fallback: If name became empty (user typed only "🗓️"), use the emoji as name
            if (!namePart) namePart = icon || 'New Folder';

            // 3. Sanitize
            const cleanName = sanitize(namePart);
            
            // 4. Update Path
            tempPath = path.join(tempPath, cleanName);

            // 5. Create Directory
            if (fs.existsSync(tempPath)) {
                if (parts.length === 1) existing.push(cleanName);
            } else {
                fs.mkdirSync(tempPath);
                if (parts.length === 1) created.push(cleanName);
            }

            // 6. Set Icon (if one was found)
            if (icon) {
                const configPath = path.join(tempPath, 'config.json');
                let conf = {};
                
                // Read existing config to avoid overwriting other settings (like locks)
                if (fs.existsSync(configPath)) {
                    try { conf = JSON.parse(fs.readFileSync(configPath)); } catch(e){}
                }
    ...
    // UPDATE CACHE
    rebuildCache(currentAbs, currentRel);

    let msg = "";
    if (created.length > 0) msg += `✅ Created: ${created.join(', ')}\n`;
    if (existing.length > 0) msg += `⚠️ Existed: ${existing.join(', ')}`;
    if (!msg) msg = "✅ Folders processed with icons.";

    return cleanReply(ctx, msg, buildKeyboard(currentRel, userPages[ctx.chat.id] || 0));
});

// 📜 CREATE PLAYLIST / INDEX (With Auto-Deep Link)
bot.command('mklist', (ctx) => {
    if (!isAdmin(ctx)) return;
    
    // 1. Parse Input
    let rawText = ctx.message.text.replace('/mklist', '').trim();
    if (!rawText) return ctx.reply("❌ Usage: /mklist 🎬 Name <Link>");
    ...
    if (match) {
        customIcon = match[0];
        rawText = rawText.replace(customIcon, '').trim();
    }

    // 3. Detect Link vs Interactive
    const parts = rawText.split(/\s+/);
    const lastPart = parts[parts.length - 1];
    const linkData = parseLink(lastPart);
    const currentRel = userPaths[ctx.chat.id] || '';

    // --- CASE A: DIRECT LINK ---
    if (linkData) {
        const name = rawText.slice(0, rawText.lastIndexOf(lastPart)).trim();
        ...
        // ⭐ GENERATE DEEP LINK
        const fileRelPath = path.join(currentRel, name); // Path to the item
        const deepLink = generateDeepLink(ctx, fileRelPath, 'file');

        return ctx.replyWithHTML(
            `✅ <b>Playlist Created!</b>\n` +
            `📂 Name: ${customIcon || ''} <b>${name}</b>\n` +
            `🔗 <b>Link:</b> ${deepLink}`
        );
    }

    // --- CASE B: INTERACTIVE ---
    const name = sanitize(rawText);
    pendingPlaylist[ctx.from.id] = {
        name: name,
        folder: currentRel,
        icon: customIcon
    };
    ...
    return ctx.reply(
        `📝 <b>Creating Playlist: "${customIcon || ''} ${name}"</b>\n` +
        `Please send/forward the menu message now.`
    );
});
...
// 📋 BATCH LIST (Now behaves like Range/Sequence)
// Usage: /batchlist <StartLink> <EndLink>
bot.command('batchlist', async (ctx) => {
    if (!isAdmin(ctx)) return;
    
    // 1. Parse Links
    const rawText = ctx.message.text.replace('/batchlist', '').trim();
    const parts = rawText.split(/(https?:\/\/[^\s]+)/g).filter(p => p.startsWith('http'));

    if (parts.length !== 2) {
        return ctx.reply("❌ Usage: /batchlist <StartLink> <EndLink>\n(This will add all files in between)");
    }

    const startLink = parts[0];
    const endLink = parts[1];

    const firstInfo = parseLink(startLink);
    const lastInfo = parseLink(endLink);

    // 2. Validate Range
    if (!firstInfo || !lastInfo) return ctx.reply("❌ Invalid Links.");
    if (firstInfo.chat_id !== lastInfo.chat_id) return ctx.reply("❌ Links must be from the same channel.");
    if (lastInfo.message_id < firstInfo.message_id) return ctx.reply("❌ End link must be after Start link.");

    const statusMsg = await ctx.reply(`🔄 Processing Sequence: IDs ${firstInfo.message_id} to ${lastInfo.message_id}...`);
    const currentRel = userPaths[ctx.chat.id] || '';
    const absPath = getAbsPath(currentRel);
    ...
    for (let msgId = firstInfo.message_id; msgId <= lastInfo.message_id; msgId++) {
        ...
        if (count > 0 && count % 5 === 0) {
            await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, undefined, 
                `🔄 Processing... ${msgId - firstInfo.message_id}/${lastInfo.message_id - firstInfo.message_id}\n✅ OK: ${count} ❌ Skip: ${failed}`
            ).catch(()=>{});
        }

        let fileName = "";

        // 4. Auto-Detect Name from Telegram
        try {
            const tempMsg = await ctx.telegram.copyMessage(ctx.chat.id, firstInfo.chat_id, msgId, { disable_notification: true });
            
            // Uses the getNameFromMessage helper we added earlier
            // If that function is missing, it falls back to "File_ID"
            if (typeof getNameFromMessage === 'function') {
                fileName = getNameFromMessage(tempMsg);
            } else {
                fileName = sanitize(tempMsg.caption?.split('\n')[0] || tempMsg.document?.file_name) || `${batchCounter}`;
            }

            // Cleanup temp message
            await ctx.telegram.deleteMessage(ctx.chat.id, tempMsg.message_id).catch(()=>{});
            await new Promise(r => setTimeout(r, 1500)); // Sleep to prevent flood limits
        } catch (e) {
            // Message might be deleted or bot isn't admin
            failed++;
            continue; // Skip this ID
        }

        // If name detection returned null/empty, use a number
        if (!fileName) fileName = String(batchCounter);

        // 5. Save & Duplicate Check
        let finalName = fileName;
        let dupCount = 1;
        while (fs.existsSync(path.join(absPath, finalName + '.json'))) {
            finalName = `${fileName}_${dupCount}`;
            dupCount++;
        }

        saveFileJson(currentRel, finalName, {
            type: 'copy',
            from_chat_id: firstInfo.chat_id,
            message_id: msgId,
            added_at: Date.now()
    ...
    return cleanReply(ctx, `✅ <b>Sequence Complete!</b>\nAdded ${count} files.\nSkipped ${failed} (deleted/error).`, buildKeyboard(currentRel, 0));
});
...
// 🗑️ MULTI-DELETE COMMAND (Supports: /del Item1 | Item2 | Item3)
bot.command('del', (ctx) => {
    if (!isAdmin(ctx)) return;

    // 1. Parse Input
    const rawText = ctx.message.text.replace('/del', '').trim();
    if (!rawText) return ctx.reply("❌ Usage: /del Name1 | Name2 | Name3");
    ...
    let notFound = [];
    let errors = [];
    let filesRemovedCount = 0; // To update metaData.total_files

    // 2. Loop through items
    items.forEach(name => {
        const cleanName = sanitize(name);
        
        // Safety: Prevent deleting system files
        if (SYSTEM_FILES.includes(cleanName) || SYSTEM_FILES.includes(cleanName + '.json')) {
            errors.push(`${cleanName} (System File)`);
            return;
        }

        const pDir = path.join(currentAbs, cleanName);
        const pFile = path.join(currentAbs, cleanName + '.json');

        try {
            // A. Check if Folder
            if (fs.existsSync(pDir) && fs.lstatSync(pDir).isDirectory()) {
                // Count files inside recursively before deleting (to fix stats)
                let internalCount = 0;
                const countRecursive = (d) => {
                    if (!fs.existsSync(d)) return;
                    fs.readdirSync(d, { withFileTypes: true }).forEach(i => {
                        if (i.isDirectory()) countRecursive(path.join(d, i.name));
                        else if (i.name.endsWith('.json')) internalCount++;
                    });
                };
                countRecursive(pDir);
                ...
                filesRemovedCount += internalCount;
                deletedFolders++;
            }
            // B. Check if File
    ...
    if (filesRemovedCount > 0) {
        metaData.total_files = Math.max(0, metaData.total_files - filesRemovedCount);
        saveMeta();
    }

    // 4. Update Cache (ONCE)
    // We rebuild from the current folder to reflect changes immediately
    rebuildCache(currentAbs, currentRel);
    ...
    let msg = `🗑️ <b>Delete Complete</b>\n\n`;
    if (deletedFolders > 0) msg += `📂 Folders: ${deletedFolders}\n`;
    if (deletedFiles > 0) msg += `📄 Files: ${deletedFiles}\n`;
    
    if (notFound.length > 0) msg += `\n⚠️ <b>Not Found:</b>\n${notFound.join(', ')}\n`;
    if (errors.length > 0) msg += `\n❌ <b>Errors:</b>\n${errors.join('\n')}`;

    if (deletedFiles === 0 && deletedFolders === 0 && notFound.length > 0) {
        return ctx.reply(`❌ Could not find: ${notFound.join(', ')}`);
    }

    return cleanReply(ctx, msg, { parse_mode: 'HTML', ...buildKeyboard(currentRel, 0) });
});
...
// 🎧 LISTENER: Handle Interactive Playlist (With Auto-Deep Link)
bot.use(async (ctx, next) => {
    if (ctx.from && pendingPlaylist[ctx.from.id] && ctx.message && !ctx.message.text?.startsWith('/')) {
        const setup = pendingPlaylist[ctx.from.id];
        
        // Save File
        saveFileJson(setup.folder, setup.name, {
            type: 'index',
            icon: setup.icon,
        ...
        rebuildCache(getAbsPath(setup.folder), setup.folder);
        delete pendingPlaylist[ctx.from.id];
        
        // ⭐ GENERATE DEEP LINK
        const fileRelPath = path.join(setup.folder, setup.name);
        const deepLink = generateDeepLink(ctx, fileRelPath, 'file');

        return ctx.replyWithHTML(
            `✅ <b>Playlist Created!</b>\n` +
            `📂 Name: ${setup.icon || ''} <b>${setup.name}</b>\n` +
            `🔗 <b>Link:</b> ${deepLink}`
        );
    }
    await next();
...
bot.action(/^del_msg(?::(\d+))?$/, async (ctx) => {
    const chatId = ctx.chat.id;
    const msgId = ctx.callbackQuery.message.message_id; // The file message
    const timerMsgId = ctx.match[1]; // The text message

    try {
        await ctx.deleteMessage().catch(() => {});
        if (timerMsgId) await ctx.telegram.deleteMessage(chatId, parseInt(timerMsgId)).catch(() => {});
        
        // ⭐ NEW: Remove from Database manually
        const db = JSON.parse(fs.readFileSync(PENDING_DELETES_FILE, 'utf8'));
        const key1 = `${chatId}:${msgId}`;
        const key2 = `${chatId}:${timerMsgId}`;
        
        if (db[key1]) delete db[key1];
        if (db[key2]) delete db[key2];
...
bot.on('text', async (ctx) => {
    ...
    // 1. CHECK FOR PENDING UPLOADS (Admin)
    ...
    // 1. Check if it matches a Folder in the current directory
    ...
    if (matchedFolder) {
        ...
        // If NOT a folder, check other possibilities:
        
        // 2. Check if it matches a File in the current directory
        const matchedFile = files.find(f => `${f.icon} ${f.name}` === text);
        
        if (matchedFile) {
            cleanText = matchedFile.name;
            isFileRequest = true;
        }
        // 3. Check Navigation Buttons (Stop here)
        else if (['ℹ️ Help', '⬅️ Back', '🏠 Index', 'Next ➡️', '⬅️ Prev'].some(b => text.startsWith(b))) {
            return; 
        }
        // 4. Check if it LOOKS like a folder (Icon + Name) - useful for global search jumps
        else if (text.startsWith(FOLDER_ICON)) {
            cleanText = text.replace(FOLDER_ICON, '').trim();
            isFolderRequest = true;
        }
        // 5. Fallback cleanup (Basic text)
        else {
            cleanText = text.replace(new RegExp(`^(${FOLDER_ICON}|${FILE_ICON})\\s*`), '').trim();
        }
    }

    // 🔴 LOCK CHECK (Only works if found in current dir)
    if (matchedFolder && matchedFolder.locked_channel && !isAdmin(ctx)) {
        try {
    ...
    // 2. FOLDER NAVIGATION (Standard browsing - Current Folder)
    ...
    if (fs.existsSync(targetDir) && fs.lstatSync(targetDir).isDirectory()) {
        ...
        return openFolder(ctx, newRel);
    }

    // 3. FILE SENDING (Standard browsing - Current Folder)
    ...
    // ⭐ 4. GLOBAL EXACT LOOKUP (The Fix for Search Navigation)
    if (isFolderRequest || isFileRequest) {
        const globalMatches = findPathRecursively(DATA_DIR, cleanText).filter(r => r.name === cleanText);

        if (globalMatches.length > 0) {
            let bestMatch = globalMatches[0];
            
            if (isFolderRequest) {
                const f = globalMatches.find(x => x.isDirectory);
                if (f) bestMatch = f;
            ...
            if (bestMatch.isDirectory) {
                const isAllowed = await checkFolderLock(ctx, getAbsPath(bestMatch.relPath));
                if (!isAllowed) return;
                logUserAction(ctx, 'NAVIGATE_SEARCH', cleanText);
                return openFolder(ctx, bestMatch.relPath);
            } else {
                const parentDir = path.dirname(getAbsPath(bestMatch.relPath));
                const isAllowed = await checkFolderLock(ctx, parentDir);
                if (!isAllowed) return;
                logUserAction(ctx, 'DOWNLOAD_SEARCH', cleanText);
                return sendFileToUser(ctx, getAbsPath(bestMatch.relPath), bestMatch.name);
            }
        }
    }
    ...
    if (isIgnored(cleanText)) {
        return ctx.reply(`⚠️ "${cleanText}" is a common folder name. Please browse manually or search for the Movie/Series title.`);
    }

    logUserAction(ctx, 'SEARCH', cleanText);
    
    const results = findPathRecursively(DATA_DIR, cleanText);
    if (results.length === 0) return ctx.reply(`❌ No results found for "${cleanText}".`);
    ...
    if (results.length === 1) {
        ...
        if (topMatch.isDirectory) {
            return openFolder(ctx, targetPath);
        } else {
             userPaths[ctx.chat.id] = targetPath;
             userPages[ctx.chat.id] = 0;
             const displayPath = formatDisplayPath(targetPath);
             return cleanReply(ctx, `📂 Opened: ${displayPath}`, buildKeyboard(targetPath, 0));
        }
    }

    searchSession[ctx.chat.id] = {
        query: cleanText,
        results: results,
        page: 0
    };
...
async function sendFileToUser(ctx, absPath, displayName) {
    try {
        ...
        if (data.type === 'index') {
            ...
            // We use copyMessage so it preserves formatting and buttons if you had them
            await ctx.telegram.copyMessage(ctx.chat.id, sourceChat, data.message_id);
            // We DO NOT set a delete timer for menus, so users can read the list comfortably.
            return;
        }
...
// 📂 HELPER: Centralized Folder Opening (Handles Previews & Navigation)
async function openFolder(ctx, relPath) {
    const absPath = getAbsPath(relPath);
    
    // 1. Update Session
    userPaths[ctx.chat.id] = relPath;
    userPages[ctx.chat.id] = 0;

    // 2. Check for Preview Image (preview.json)
    const previewFile = path.join(absPath, 'preview.json');
    if (fs.existsSync(previewFile)) {
        try {
            const pData = JSON.parse(fs.readFileSync(previewFile, 'utf8'));
            const sourceChat = pData.from_chat_id || pData.chat_id;
            
            // Delete user's previous input to keep chat clean
            try { await ctx.deleteMessage(); } catch(e){}

            // Send Copy with Keyboard
            const sentMsg = await ctx.telegram.copyMessage(ctx.chat.id, sourceChat, pData.message_id, {
                ...buildKeyboard(relPath, 0)
            });
            
            // Auto-delete bot message
            scheduleDelete(ctx.chat.id, sentMsg.message_id, metaData.timers.bot_msg);
            return;
        } catch (err) {
    ...
    // 3. Normal Folder Open (No Preview)
    const displayPath = formatDisplayPath(relPath);
    
    // Attempt to find folder icon
    const parentDir = path.dirname(relPath);
    const { folders } = getDirContents(parentDir);
    const folderObj = folders.find(f => f.name === path.basename(relPath));
    const icon = folderObj ? folderObj.icon : FOLDER_ICON;

    // We merge the keyboard object with the parse_mode option
    return cleanReply(ctx, 
        `${icon} Path: ${displayPath}`, 
        { parse_mode: 'HTML', ...buildKeyboard(relPath, 0) }
    );
}

// Launch
bot.launch().then(() => {
    console.log('🚀 Collections Bot Started');
    restoreTimers(); // <--- ⭐ ADD THIS HERE
});
